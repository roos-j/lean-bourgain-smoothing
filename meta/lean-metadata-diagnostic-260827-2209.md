...
Date and time: 2026-08-27 22:09 PDT
...
- `def:main-interaction-data` is now `Completed` as `mainInteractionData`.
- `lem:main-interaction-admissible`, `lem:trilinear-localization`, and
  `lem:localized-sobolev-decay` are now `Statement completed` as
  `mainInteractionDataAdmissible`, `trilinearFormSpatialLocalization`, and
  `localizedNegativeSobolevDecay`.
- `prop:dyadic-linfty-decay` is now `Statement completed` as
...
the required `\\lean{...}` commands for the five non-`Todo` labels would create
